export function calculateCost(data) {
  ...
  const Worker_coef = (ClusterType) => {
    if (ClusterType === 2) return 0.2;
    if (ClusterType === 1) return Help_coef;
    return Help_coef * 0.7 + 0.3 * 0.2;
  };

  let ST_WORKER_HOURS =
    Worker_BASE[data._WorkSt - 1] * Worker_coef(data._ClusterType);
  let SP_WORKER_HOURS =
    Worker_BASE[data._WorkSp - 1] * Worker_coef(data._ClusterType) * 2;
